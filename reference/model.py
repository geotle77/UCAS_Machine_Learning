...
class Model(nn.Module):
        ...
        if self.multi:
            if self.hp.adapter_name == 'cross-atten':
                ...
                if self.hp.use_prefix_p:
                    ...
                    outputs = self.T5_encoder.t5_model.generate(t5_input_id, attention_mask=attention_mask,
                                                                visual=(visual, visual_seq),
                                                                acoustic=(acoustic, acoustic_seq),
    ...
    def forward(self, sentences, t5_input_id, t5_att_mask, t5_labels, ids, visual=None, acoustic=None, v_len=None,
                a_len=None):
        if self.multi:
            if self.hp.adapter_name == 'cross-atten':
                ...
                enc_output = self.T5_encoder(sentences, t5_input_id, t5_att_mask, t5_labels,
                                             visual=(visual, visual_seq), acoustic=(acoustic, acoustic_seq))
            else:
                ...
                if self.hp.use_prefix_p:
                    ...
                    prefix_attention_mask = torch.ones(batch_size, self.pre_seq_len).to(DEVICE)
                    attention_mask = torch.cat((prefix_attention_mask, t5_att_mask), dim=1)
                    enc_output = self.T5_encoder(sentences, t5_input_id, attention_mask, t5_labels,
                                                 prompt_key_values=prompt_key_values, visual=(visual, visual_seq),
                                                 acoustic=(acoustic, acoustic_seq))
                else:
                    enc_output = self.T5_encoder(sentences, t5_input_id, t5_att_mask, t5_labels,
                                                 visual=(visual, visual_seq), acoustic=(acoustic, acoustic_seq))
            ...
            enc_output = self.T5_encoder(sentences, t5_input_id, t5_att_mask,
                                         t5_labels)  # (batch_size, seq_len, emb_size)
