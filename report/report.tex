    ...
    \section{解决方法}
        ...
        \subsection{Transformer\_Based模型}
            ...
            \begin{enumerate}
                ...
                保证输出的维度均为隐藏层的维度：
                \begin{align}
                    \textbf{U}_m' = \textrm{Conv1D}(\textbf{U}_m',k_m) \in \mathbb{R} ^{N \times d},\quad m \in \{t,a,v\}
                \end{align}
                ...
                \begin{align}
                    \textbf{s}_j = \textbf{V}_s \textbf{o}(s_j) \in \mathbb{R}^d,\quad j = 1, 2, \cdots,M
                \end{align}
                其中$M$是说话人的数量，$\textbf{V}_s \in \mathbb{R}^{d\times M}$是一个可训练的说话者嵌入矩阵，
                $\textbf{o}(s_j)$是一个关于说话者$s_j$的独热编码向量，即向量中第$j$个元素是1，其余为0。
                ...
                \begin{align}
                    \textbf{H}_{m\rightarrow m} = \textrm{Transformer}(\textbf{H}_{m},\textbf{H}_{m},\textbf{H}_{m}) \in \mathbb{R}^{N\times d}
                \end{align}
                
                对于模态间，我们将一个模态的输入信息作为Transforner编码器的Q，另一个模态的输入信息作为Transforner编码器的K，V。
                \begin{align}
                    \textbf{H}_{n\rightarrow m} = \textrm{Transformer}(\textbf{H}_{m},\textbf{H}_{n},\textbf{H}_{n}) \in \mathbb{R}^{N\times d}
                \end{align}
                ...
                \begin{align}
                    ...
                    &\mathcal{L}_2 = -\frac{1}{N}\sum_{m \in {t,a,v}}\sum_{i = 1}^{N}\sum_{j = 1}^{C}\textbf{y}_{i, j}\log(\hat{\textbf{y}_{mi, j}})\\
