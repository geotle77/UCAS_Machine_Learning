...
class Transformer_Based_Model(nn.Module):
    def __init__(self, D_text, D_visual, D_audio, n_head,
        ...
        self.speaker_embeddings = nn.Embedding(n_speakers+1, hidden_dim, self.padding_idx)
        ...
        self.t_t_gate = Unimodal_GatedFusion(hidden_dim)
        self.a_t_gate = Unimodal_GatedFusion(hidden_dim)
        self.v_t_gate = Unimodal_GatedFusion(hidden_dim)

        self.a_a_gate = Unimodal_GatedFusion(hidden_dim)
        self.t_a_gate = Unimodal_GatedFusion(hidden_dim)
        self.v_a_gate = Unimodal_GatedFusion(hidden_dim)

        self.v_v_gate = Unimodal_GatedFusion(hidden_dim)
        self.t_v_gate = Unimodal_GatedFusion(hidden_dim)
        self.a_v_gate = Unimodal_GatedFusion(hidden_dim)
